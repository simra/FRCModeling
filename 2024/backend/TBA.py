...
class TBA:
    ...
    def fetch_event_teams(self, event_key):
        return self.api_instance.get_event_teams(event_key, _request_timeout=self._request_timeout)
    ...
    def fetch_matches(self, team_key = 'frc492', if_modified_since=''):
        ...
        try:
            ...
            for e in events:
                ...
                matches = self.api_instance.get_event_matches(e.key, _request_timeout=self._request_timeout)
                result += matches
                #for m in matches:
